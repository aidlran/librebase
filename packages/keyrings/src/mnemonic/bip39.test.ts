...
english.forEach(([entropyHex, mnemonic, seedHex], i) => {
  describe(`Vector ${i + 1} (${mnemonic.slice(0, 30).trim()}...)`, () => {
    ...
    test('entropyToMnemonic', async () => {
      expect(await entropyToMnemonic(entropy)).toEqual(words);
    });

    test('mnemonicToEntropy', async () => {
      expect(await mnemonicToEntropy(words)).toEqual(entropy);
    });
...
test('Catch invalid lengths', () => {
  ...
  for (let i = 0; i < 33; i++) {
    if (i < 16 || i > 32 || i % 4 != 0) {
      ...
      expect(entropyToMnemonic(truncatedEntropy)).rejects.toThrow('Invalid entropy length');
    }
    if (i < 12 || i > 24 || i % 3 != 0) {
      const truncatedMnemonic = mnemonic.slice(0, i);
      expect(mnemonicToEntropy(truncatedMnemonic)).rejects.toThrow('Invalid mnemonic length');
    }
  }
});
...
test('Catch non-wordlist words', () => {
  ...
  expect(mnemonicToEntropy(mnemonic)).rejects.toThrow('not in wordlist');
});
...
test('Catch invalid checksum', () => {
  ...
  expect(mnemonicToEntropy(badMnemonic)).rejects.toThrow('Invalid checksum');
});
