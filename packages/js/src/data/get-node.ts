...
export function getNode(this: Injector) {
  const parse = parseSerializedNode.bind(this(createNode));
  return (hash: Uint8Array) => {
    return this(channelModule).getNode(hash, (data) => parse(data, hash));
  };
}

async function parseSerializedNode(this: () => Node, data: RetrievedNodeData, hash: Uint8Array) {
  const node = this().setMediaType(data[0]);
